...
int getOrientation() {
  readAccelerometer();
  float roll = atan2(ax, ay) * 180.0 / PI;  // Calibrated: using AX/AY instead of AX/AZ
  if (roll < 0) roll += 360;
  if (roll >= 319 || roll < 50) return 270; //0      // Upright (0 deg)
  if (roll >= 50 && roll < 139) return 0; //90    // Right side down
  if (roll >= 228 && roll < 319) return 180; //180 // Upside down
  return 90; 
}
...
void displayClockMode(bool forceRefresh = false) {
  ...
  if (orientation == 0) {
    // At physical 0° (upright): show dot time
    ...
    // Swapped matrices: hours on matrix 1, minutes on matrix 0
    // Both matrices display digits upside down (flipped 180 degrees)
    drawTwoDigitsFlippedSwapped(1, clockHours);
    drawTwoDigitsFlippedSwapped(0, clockMinutes);
...
void drawTwoDigitsMirrored(int matrix, int number) {
  if (number < 0) number = 0;
  if (number > 99) number = 99;
  int tens = number / 10;
  int ones = number % 10;
  lc.clearDisplay(matrix);
  // Mirror horizontally: flip columns (col 0->2, 1->1, 2->0)
  // and flip rows (row 0->4, 1->3, 2->2, 3->1, 4->0)
  // Also swap tens and ones positions for proper mirrored display
  for (int col = 0; col < 3; col++) {
    int mirroredCol = 2 - col;  // Flip column: 0->2, 1->1, 2->0
    for (int row = 0; row < 5; row++) {
      int mirroredRow = 4 - row;  // Flip row: 0->4, 1->3, 2->2, 3->1, 4->0
      // Swapped: ones digit now goes to left side (cols 1-3), tens to right (cols 5-7)
      if (smallDigit[ones][col] & (1 << row))
        lc.setLed(matrix, mirroredRow + 1, mirroredCol + 1, true);
      if (smallDigit[tens][col] & (1 << row))
        lc.setLed(matrix, mirroredRow + 1, mirroredCol + 5, true);
    }
  }
}

void drawTwoDigitsFlipped(int matrix, int number) {
  if (number < 0) number = 0;
  if (number > 99) number = 99;
  int tens = number / 10;
  int ones = number % 10;
  lc.clearDisplay(matrix);
  // Flip digits 180 degrees (upside down) without swapping tens/ones
  // This compensates for 180 degree display rotation
  for (int col = 0; col < 3; col++) {
    int flippedCol = 2 - col;  // Flip column: 0->2, 1->1, 2->0
    for (int row = 0; row < 5; row++) {
      int flippedRow = 4 - row;  // Flip row: 0->4, 1->3, 2->2, 3->1, 4->0
      // Normal order: tens on left (cols 1-3), ones on right (cols 5-7)
      if (smallDigit[tens][col] & (1 << row))
        lc.setLed(matrix, flippedRow + 1, flippedCol + 1, true);
      if (smallDigit[ones][col] & (1 << row))
        lc.setLed(matrix, flippedRow + 1, flippedCol + 5, true);
    }
  }
}

void drawTwoDigitsSwapped(int matrix, int number) {
  if (number < 0) number = 0;
  if (number > 99) number = 99;
  int tens = number / 10;
  int ones = number % 10;
  lc.clearDisplay(matrix);
  // Swap digit positions: ones on left, tens on right
  // This fixes the swapped appearance at 90 degrees
  for (int col = 0; col < 3; col++) {
    for (int row = 0; row < 5; row++) {
      // Swapped: ones digit goes to left side (cols 1-3), tens to right (cols 5-7)
      if (smallDigit[ones][col] & (1 << row))
        lc.setLed(matrix, row + 1, col + 1, true);
      if (smallDigit[tens][col] & (1 << row))
        lc.setLed(matrix, row + 1, col + 5, true);
...
void drawTwoDigitsFlippedSwapped(int matrix, int number) {
  if (number < 0) number = 0;
  if (number > 99) number = 99;
  int tens = number / 10;
  int ones = number % 10;
  lc.clearDisplay(matrix);
  // Flip digits 180 degrees AND swap digit positions (ones on left, tens on right)
  for (int col = 0; col < 3; col++) {
    int flippedCol = 2 - col;  // Flip column: 0->2, 1->1, 2->0
    for (int row = 0; row < 5; row++) {
      int flippedRow = 4 - row;  // Flip row: 0->4, 1->3, 2->2, 3->1, 4->0
...
void displayFlipCounter() {
  ...
  // At 0° (upright): show mirrored count on top matrix (MATRIX_TOP)
  // At 180° (upside down): show normal count on bottom matrix (MATRIX_BOTTOM)
  // At 90° and 270°: show mirrored count on both matrices
  if (currentGravity == 0) {
    Serial.println("  -> Using drawTwoDigitsMirrored(MATRIX_TOP)");
    drawTwoDigitsMirrored(MATRIX_TOP, flipCount);
  } else if (currentGravity == 180) {
    Serial.println("  -> Using drawTwoDigits(MATRIX_BOTTOM)");
    drawTwoDigits(MATRIX_BOTTOM, flipCount);
  } else if (currentGravity == 90) {
    Serial.println("  -> Using drawTwoDigitsMirrored(both)");
    drawTwoDigitsMirrored(0, flipCount);
    drawTwoDigitsMirrored(1, flipCount);
  } else if (currentGravity == 270) {
    Serial.println("  -> Using drawTwoDigits(both)");
    drawTwoDigits(0, flipCount);
    drawTwoDigits(1, flipCount);
  } else {
    Serial.println("  -> Unknown gravity!");
  }
}
...
void loop() {
  ...
  if (currentMode == MODE_HOURGLASS) {
    ...
    checkFlip();
    // Update display if count changed or orientation changed
    ...
    if (flipCount != lastDisplayedFlipCount || currentGravity != lastDisplayedFlipOrientation) {
      Serial.print("FlipCounter display: count=");
      Serial.print(flipCount);
      Serial.print(" gravity=");
      Serial.println(currentGravity);
      displayFlipCounter();
