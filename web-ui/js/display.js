...
class Display {
    ...
    normalizeMatrix(data) {
        ...
        // Support Hex String format (16 chars)
        if (typeof data === 'string' && data.length === 16) {
            ...
            for (let row = 0; row < 8; row++) {
                ...
                // Validate hexByte/byteVal
                if (isNaN(byteVal) || !/^[0-9A-Fa-f]{2}$/.test(hexByte)) {
                    console.error('Invalid hex byte in matrix data:', hexByte);
                    return empty;
                }
