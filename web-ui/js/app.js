class App {
    constructor() {
        ...
        this._boundUpdateUI = (e) => this.updateUI(e.detail);
        this.init();
    }
    ...
    attachListeners() {
        document.getElementById('btn-connect')?.addEventListener('click', () => this.connectDevice());
        document.getElementById('btn-disconnect')?.addEventListener('click', () => this.disconnectDevice());
        ...
        window.addEventListener('statusUpdate', this._boundUpdateUI);
    ...
    hasDataChanged(newData) {
        ...
        return JSON.stringify(this.lastUpdateData) !== JSON.stringify(newData);
    }
    ...
    async updateStatus() {
        ...
        try {
            ...
            if (!this.hasDataChanged(data)) return;
            
            this.lastUpdateData = data;
            if (data.status) this.updateUI(data.status, { skipOrientation: true });
            
            // Pass mode, time, orientation and flipCount to display for special handling
            const mode = data.status?.mode;
            const time = data.status?.time;
            const orientation = data.orientation?.angle ?? data.status?.orientation;
            const flipCount = data.status?.flipCount;
            if (data.display) display.updateFromAPI(data.display, mode, time, orientation, flipCount);
            
            if (data.orientation !== undefined) {
                const orientationData = typeof data.orientation === 'number'
                    ? { angle: data.orientation }
                    : data.orientation;
                display.updateOrientation(orientationData);
            }
            this.setConnectionStatus(true);
        } catch (error) {
    ...
    updateUI(status, options = {}) {
        ...
        if (!options.skipOrientation && status.orientation !== undefined && display) {
